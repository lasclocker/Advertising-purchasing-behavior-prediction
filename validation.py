...
table1 = sc.textFile("/data/validation/monitorData/*")
def f1(x):
	lt = x.split('^')
	userID = lt[0]
	return userID
...
result4 = result.map(lambda s:(s,1)).leftOuterJoin(result3) #result leftOuterJoin data of IMPNums
...
result6.saveAsTextFile('/user/team322/junli_testFeature')
t2 = time.ctime()
print 'Starting:\t%s\nEnding:\t%s'%(t1,t2)
