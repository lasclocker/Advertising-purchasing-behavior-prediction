...
import time
t1 = time.ctime()
sc = SparkContext(appName="DianShang3")
table1 = sc.textFile("/user/team322/junli_trueTestFeature/*")
def f1(line):
	line = str(line).replace('(','').replace(')','').replace('None','0')
	userID = line.split(',')[0]
	return userID
user = table1.map(f1).collect()#select the users of test data
result6 = sc.textFile("/user/team322/junli_trainFeature3/*")
...
result7 = sc.textFile("/user/team322/junli_trueTestFeature/*")
...
sc.parallelize(userID).saveAsTextFile('/user/team322/solution_t') #create a parallelized collection and save it 
t2 = time.ctime()
print 'Starting:\t%s\nEnding:\t%s'%(t1,t2)